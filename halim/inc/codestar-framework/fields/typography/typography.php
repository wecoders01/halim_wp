    ...
    class CSF_Field_typography extends CSF_Fields {
        ...
        public function render() {
            ...
            if ( !empty( $args['font_family'] ) ) {
                ...
                echo '<div class="csf--title">' . esc_html__( 'Font Family', 'halim' ) . '</div>';
                echo $this->create_select( array( $this->value['font-family'] => $this->value['font-family'] ), 'font-family', esc_html__( 'Select a font', 'halim' ) );
                echo '</div>';
            }
            ...
            if ( !empty( $args['backup_font_family'] ) ) {
                ...
                echo '<div class="csf--title">' . esc_html__( 'Backup Font Family', 'halim' ) . '</div>';
                echo $this->create_select( apply_filters( 'csf_field_typography_backup_font_family', array(
                    ...
                    'Georgia, serif',
                    'Palatino Linotype',
                ) ), 'backup-font-family', esc_html__( 'Default', 'halim' ) );
                echo '</div>';
            }
            ...
            if ( !empty( $args['font_weight'] ) || !empty( $args['font_style'] ) ) {
                ...
                echo '<div class="csf--title">' . esc_html__( 'Font Style', 'halim' ) . '</div>';
                echo '<select class="csf--font-style-select" data-placeholder="Default">';
                echo '<option value="">' . ( !$this->chosen ? esc_html__( 'Default', 'halim' ) : '' ) . '</option>';
                ...
                if ( !empty( $args['extra_styles'] ) ) {
                    ...
                    echo ( !$this->chosen ) ? '<div class="csf--title">' . esc_html__( 'Load Extra Styles', 'halim' ) . '</div>' : '';
                    $placeholder = ( $this->chosen ) ? esc_html__( 'Load Extra Styles', 'halim' ) : esc_html__( 'Default', 'halim' );
            ...
            if ( !empty( $args['subset'] ) ) {
                ...
                echo '<div class="csf--title">' . esc_html__( 'Subset', 'halim' ) . '</div>';
                $subset = ( is_array( $this->value['subset'] ) ) ? $this->value['subset'] : array_filter( (array) $this->value['subset'] );
                echo $this->create_select( $subset, 'subset', esc_html__( 'Default', 'halim' ), $args['multi_subset'] );
                echo '</div>';
            }
            ...
            if ( !empty( $args['text_align'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Text Align', 'halim' ) . '</div>';
                echo $this->create_select( array(
                    'inherit' => esc_html__( 'Inherit', 'halim' ),
                    'left'    => esc_html__( 'Left', 'halim' ),
                    'center'  => esc_html__( 'Center', 'halim' ),
                    'right'   => esc_html__( 'Right', 'halim' ),
                    'justify' => esc_html__( 'Justify', 'halim' ),
                    'initial' => esc_html__( 'Initial', 'halim' ),
                ), 'text-align', esc_html__( 'Default', 'halim' ) );
                echo '</div>';
            }
            ...
            if ( !empty( $args['font_variant'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Font Variant', 'halim' ) . '</div>';
                echo $this->create_select( array(
                    'normal'         => esc_html__( 'Normal', 'halim' ),
                    'small-caps'     => esc_html__( 'Small Caps', 'halim' ),
                    'all-small-caps' => esc_html__( 'All Small Caps', 'halim' ),
                ), 'font-variant', esc_html__( 'Default', 'halim' ) );
                echo '</div>';
            }
            ...
            if ( !empty( $args['text_transform'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Text Transform', 'halim' ) . '</div>';
                echo $this->create_select( array(
                    'none'       => esc_html__( 'None', 'halim' ),
                    'capitalize' => esc_html__( 'Capitalize', 'halim' ),
                    'uppercase'  => esc_html__( 'Uppercase', 'halim' ),
                    'lowercase'  => esc_html__( 'Lowercase', 'halim' ),
                ), 'text-transform', esc_html__( 'Default', 'halim' ) );
                echo '</div>';
            }
            ...
            if ( !empty( $args['text_decoration'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Text Decoration', 'halim' ) . '</div>';
                echo $this->create_select( array(
                    'none'               => esc_html__( 'None', 'halim' ),
                    'underline'          => esc_html__( 'Solid', 'halim' ),
                    'underline double'   => esc_html__( 'Double', 'halim' ),
                    'underline dotted'   => esc_html__( 'Dotted', 'halim' ),
                    'underline dashed'   => esc_html__( 'Dashed', 'halim' ),
                    'underline wavy'     => esc_html__( 'Wavy', 'halim' ),
                    'underline overline' => esc_html__( 'Overline', 'halim' ),
                    'line-through'       => esc_html__( 'Line-through', 'halim' ),
                ), 'text-decoration', esc_html__( 'Default', 'halim' ) );
                echo '</div>';
            }
            ...
            if ( !empty( $args['font_size'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Font Size', 'halim' ) . '</div>';
            ...
            if ( !empty( $args['line_height'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Line Height', 'halim' ) . '</div>';
                echo '<div class="csf--input-wrap">';
            ...
            if ( !empty( $args['letter_spacing'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Letter Spacing', 'halim' ) . '</div>';
                echo '<div class="csf--input-wrap">';
            ...
            if ( !empty( $args['word_spacing'] ) ) {
                echo '<div class="csf--block">';
                echo '<div class="csf--title">' . esc_html__( 'Word Spacing', 'halim' ) . '</div>';
                echo '<div class="csf--input-wrap">';
            ...
            if ( !empty( $args['color'] ) ) {
                ...
                echo '<div class="csf--title">' . esc_html__( 'Font Color', 'halim' ) . '</div>';
            ...
            if ( !empty( $args['custom_style'] ) ) {
                ...
                echo '<div class="csf--title">' . esc_html__( 'Custom Style', 'halim' ) . '</div>';
        ...
        public function enqueue() {
            ...
            if ( !wp_script_is( 'csf-webfontloader' ) ) {
                ...
                wp_enqueue_script( 'csf-webfontloader', 'https://cdn.jsdelivr.net/npm/webfontloader@1.6.28/webfontloader.min.js', array( 'halim' ), '1.6.28', true );
                ...
                if ( !empty( $customwebfonts ) ) {
                    $webfonts['custom'] = array(
                        'label' => esc_html__( 'Custom Web Fonts', 'halim' ),
                        'fonts' => $customwebfonts,
                    );
                }

                $webfonts['safe'] = array(
                    'label' => esc_html__( 'Safe Web Fonts', 'halim' ),
                ...
                $webfonts['google'] = array(
                    'label' => esc_html__( 'Google Web Fonts', 'halim' ),
                ...
                wp_localize_script( 'halim', 'csf_typography_json', array(
                    'webfonts'      => $webfonts,
