      ...
      <div class="row">
         <div class="col-xl-12">
            <div class="portfolio-menu mb-40 text-center">
               <button class="active" data-filter="*"><?php esc_html_e( 'ALL', 'halim' )?></button>
               <?php
                   $portfolio_menus = get_terms( 'portfolio-cat' );
                   foreach ( $portfolio_menus as $portfolio_menu ):
               ?>
                  <button data-filter=".<?php echo esc_attr( $portfolio_menu->slug ); ?>" class=""><?php echo $portfolio_menu->name; ?> </button>
               <?php endforeach;?>
            </div>
         </div>
      </div>
